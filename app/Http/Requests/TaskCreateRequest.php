...
class TaskCreateRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'images' => 'array',
            'images.*' => 'mimes:jpg,jpeg,png,pdf|max:10000',
            'verifier_driver' => [
                'nullable',
                Rule::in(config('tasks.verifier.drivers')),
            ],
            'verifier_types' => 'required_with:verifier_driver|array',
            'verifier_types.*' => [
                Rule::in(config('tasks.verifier.types')[$this->verifier_driver] ?? []),
                'distinct:ignore_case',
            ],
            'tweet_words' => [
...
//                Rule::requiredIf(fn () => $verifierTypes->contains('twitter_reply')),
    ...
    {
        return [
            'twitter_space.twitter_space' => 'Space link is invalid',
            'twitter_tweet.twitter_tweet' => 'Tweet link is invalid',
            'twitter_follow.twitter_handle' => 'Twitter account is invalid',
            'discord_invite.discord_invite' => 'Discord bot is not connected to this server!',
            'telegram_invite.telegram_invite' => 'Telegram bot is not connected to this chat!',
        ];
    }
